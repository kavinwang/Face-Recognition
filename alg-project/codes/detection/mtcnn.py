...
def detect_faces(caffe_model_dir, image_path, output_dir):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            pnet, rnet, onet = create_mtcnn(sess, caffe_model_dir)
    ...
    if bounding_boxes is not None and len(bounding_boxes) > 0:
        with codecs.open(output_dir + 'bounding_boxes.txt', 'w', 'utf-8') as f:
            f.write(str(bounding_boxes))
    if points is not None and len(points) > 0:
        with codecs.open(output_dir + 'points.txt', 'w', 'utf-8') as f:
            f.write(str(points))
    ...
    if len(bounding_boxes) > 0:
        for i in range(len(bounding_boxes)):
            ...
            for p in mark:
                cv2.circle(img, (p[0], p[1]), 1, (255, 0, 0))

    misc.imsave(output_dir + 'face.png', img)
    for i in range(len(crops)):
        misc.imsave(output_dir + 'face_' + str(i) + '.png', crops[i])


def main(args):
    detect_faces(args.caffe_model_dir, args.image_path, args.output_dir)


def parse_arguments(argv):
    parser = argparse.ArgumentParser()

    parser.add_argument('--caffe_model_dir', type=str,
                        help='Directory where to read the caffe model.')
    parser.add_argument('--output_dir', type=str,
                        help='Directory write to write result')
    parser.add_argument('--image_path', type=str,
                        help='Image to detection face')
